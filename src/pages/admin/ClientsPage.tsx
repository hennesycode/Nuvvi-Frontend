...
function optionName(item?: CatalogItem) {
  return String(item?.name_city ?? item?.name_department ?? item?.country_name ?? item?.name ?? item?.label ?? item?.description ?? item?.text ?? "");
}

function optionCode(item?: CatalogItem) {
  return String(item?.city_code ?? item?.code ?? item?.abbreviation_A2 ?? item?.value ?? item?.id ?? "");
}
