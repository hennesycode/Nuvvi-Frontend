...
function ColombiaFlag() {
  return <span className="block h-4 w-6 overflow-hidden rounded-[3px] border border-[#D9ECFA] shadow-sm" aria-label="Bandera de Colombia"><span className="block h-1/2 bg-[#FCD116]" /><span className="block h-1/4 bg-[#003893]" /><span className="block h-1/4 bg-[#CE1126]" /></span>;
}
...
function ConfirmDialog({ config, onClose }: { config: ConfirmConfig; onClose: () => void }) {
  ...
  return (
    <Modal title={config.title} onClose={onClose}>
      <div className={cn("rounded-2xl p-4 text-sm font-semibold", danger ? "bg-[#FDE8E8] text-[#C94455]" : "bg-[#E8F0FE] text-[#246FC1]")}>{config.message}</div>
      <div className="mt-6 flex justify-end gap-2">
        <Button variant="outline" onClick={onClose} className="rounded-xl border-[#D9ECFA] bg-white text-[#39566F]">Cancelar</Button>
        <Button onClick={() => { config.onConfirm(); onClose(); }} className={cn("rounded-xl", danger && "bg-[#C94455] hover:bg-[#A83342]")}>{config.confirmLabel}</Button>
      </div>
    </Modal>
  );
}
...
export function ClientsPage() {
  ...
  const confirmCompanyAction = (company: Company, action: "sync" | "enable-provider" | "disable-provider" | "archive") => {
    const config: Record<typeof action, ConfirmConfig> = {
      sync: { title: "Sincronizar empresa", message: `Se consultará MATIAS y se actualizarán los identificadores y estado de ${company.legal_name}.`, confirmLabel: "Sincronizar", tone: "info", onConfirm: () => actionMutation.mutate({ id: company.id, action }) },
      "enable-provider": { title: "Habilitar en MATIAS", message: `Se llamará el endpoint de habilitación para ${company.legal_name}. Confirma que esta empresa debe quedar activa remotamente.`, confirmLabel: "Habilitar", tone: "info", onConfirm: () => actionMutation.mutate({ id: company.id, action }) },
      "disable-provider": { title: "Desactivar en MATIAS", message: "Se llamará DELETE en MATIAS. Usa esta acción solo si entiendes que puede desactivar o eliminar remotamente según Sandbox.", confirmLabel: "Desactivar", tone: "danger", onConfirm: () => actionMutation.mutate({ id: company.id, action }) },
      archive: { title: "Archivar empresa local", message: "La empresa se archivará solo en NUVVI y conservará auditoría e identificadores. No se eliminará en MATIAS.", confirmLabel: "Archivar", tone: "danger", onConfirm: () => actionMutation.mutate({ id: company.id, action }) },
    };
    setConfirmConfig(config[action]);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }} className="space-y-6">
      ...
      <CompaniesTable companies={companiesQuery.data || []} busy={busy} syncStatus={syncStatusQuery.data} onView={setSelected} onEdit={setEditing} onStats={setStatsCompany} onSync={(company) => confirmCompanyAction(company, "sync")} onEnable={(company) => confirmCompanyAction(company, "enable-provider")} onDisable={(company) => confirmCompanyAction(company, "disable-provider")} onArchive={(company) => confirmCompanyAction(company, "archive")} />
          ...
          {formStep === 1 && (
            <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Razón social" error={form.company_name && !fieldChecks.companyName ? "Escribe al menos 3 caracteres." : undefined}>
                ...
                <Field label="Celular" hint="Número colombiano de 10 dígitos.">
                  <div className="flex h-11 overflow-hidden rounded-xl border border-[#D9ECFA] bg-white focus-within:ring-2 focus-within:ring-[#4F9FF0]/20"><span className="flex items-center gap-2 border-r border-[#D9ECFA] bg-[#F8FCFF] px-3 text-sm font-bold text-[#102F4B]"><ColombiaFlag />+57</span><input inputMode="numeric" value={mobileDigits} onChange={(e) => setForm((c) => ({ ...c, mobile: `+57${onlyNumbers(e.target.value, 10)}` }))} className="min-w-0 flex-1 px-3 text-sm text-[#102F4B] outline-none" placeholder="3001234567" /></div>
                </Field>
