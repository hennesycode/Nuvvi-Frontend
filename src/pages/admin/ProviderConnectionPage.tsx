    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }} className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => syncMutation.mutate()} disabled={!canAct || busy} className="h-11 rounded-xl"><RefreshCw size={16} className="mr-2" />Sincronizar catálogos</Button>
          <Button variant="outline" onClick={() => setShowDiagnostics((current) => !current)} className="h-11 rounded-xl">Ver diagnóstico</Button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Section title="2. Configuración del ambiente">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Ambiente"><div className="grid grid-cols-2 gap-2"><button onClick={() => changeEnvironment("sandbox")} className={cn("rounded-xl border p-3 text-left text-sm", form.environment === "sandbox" ? "border-[#4F9FF0] bg-[#E8F0FE] text-[#246FC1]" : "border-[#D9ECFA]")}>Sandbox<p className="text-xs text-[#6C8398]">No envía documentos reales.</p></button><button onClick={() => changeEnvironment("production")} className={cn("rounded-xl border p-3 text-left text-sm", form.environment === "production" ? "border-[#C94455] bg-[#FDE8E8] text-[#C94455]" : "border-[#D9ECFA]")}>Producción<p className="text-xs text-[#6C8398]">Tiene efectos reales.</p></button></div></Field>
              ...
              <Field label="Tiempo máximo de espera"><input type="number" min={5} max={120} value={form.timeout_seconds} onChange={(e) => setForm((c) => ({ ...c, timeout_seconds: Number(e.target.value) }))} className={inputClass} /></Field>
              <Field label="Reintentos automáticos"><input type="number" min={0} max={5} value={form.retry_attempts} onChange={(e) => setForm((c) => ({ ...c, retry_attempts: Number(e.target.value) }))} className={inputClass} /></Field>
            </div>
          </Section>
