...
function statusStyle(status: string) {
  ...
  if (["DISABLED", "NOT_CONFIGURED", "TESTING", "INACTIVE", "PAT_REQUIRED", "PAT_ENDPOINT_NOT_FOUND", "PAT_VALIDATION_ERROR", "PAT_VALID", "MULTICOMPANY_VERIFIED", "CATALOGS_PENDING", "CATALOGS_PARTIAL", "DISCONNECTED", "pending", "partial", "CATALOGS_NOT_SYNCHRONIZED"].includes(status)) return "bg-[#FFF3E0] text-[#B97812]";
...
export function ProviderConnectionPage() {
  ...
  const membershipStatus = connection?.membership_status || (connection?.membership_plan || connection?.membership_expires_at ? "Parcialmente detectada" : "Estado no informado");
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }} className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        <div className="space-y-6">
          ...
          <Section title="Plan y consumo"><div className="grid gap-4 md:grid-cols-2"><Info label="Plan MATIAS" value={connection?.membership_plan || "No detectado"} /><Info label="Estado membresía" value={membershipStatus} /><Info label="Vigencia" value={formatDate(connection?.membership_expires_at)} /><Info label="Documentos disponibles" value={String(connection?.membership_documents_available ?? "-")} /><Info label="Documentos consumidos" value={String(connection?.membership_documents_consumed ?? "-")} /><Info label="Límite empresas/cuotas" value={String(connection?.membership_company_limit ?? "-")} /></div></Section>
        </div>

        <aside className="space-y-6">
          <Section title="Generar token MATIAS"><div className="space-y-3"><input placeholder="Correo" value={generateForm.email} onChange={(e) => setGenerateForm((c) => ({ ...c, email: e.target.value }))} className={inputClass} /><input type="password" placeholder="Contraseña temporal (no se guarda)" value={generateForm.password} onChange={(e) => setGenerateForm((c) => ({ ...c, password: e.target.value }))} className={inputClass} /><div><input placeholder="Nombre del token" value={generateForm.token_name} onChange={(e) => setGenerateForm((c) => ({ ...c, token_name: e.target.value }))} className={inputClass} /><p className="mt-1 text-xs text-[#6C8398]">MATIAS solo acepta letras sin tilde, números, espacios, guiones, guiones bajos y puntos.</p></div><input placeholder="Descripción" value={generateForm.description} onChange={(e) => setGenerateForm((c) => ({ ...c, description: e.target.value }))} className={inputClass} /><input type="number" min={1} max={365} value={generateForm.expires_in_days} onChange={(e) => setGenerateForm((c) => ({ ...c, expires_in_days: Number(e.target.value) }))} className={inputClass} />{hasValidPat && <p className="rounded-xl bg-[#D8F5EB] px-3 py-2 text-xs font-semibold text-[#178C68]">Ya existe un PAT válido para este ambiente. Conserva el token actual hasta su vencimiento; usa Rotar PAT solo si necesitas reemplazarlo antes.</p>}<Button onClick={() => generateMutation.mutate()} disabled={hasValidPat || busy || !generateForm.email || !generateForm.password} className="h-11 w-full rounded-xl"><KeyRound size={16} className="mr-2" />Iniciar sesión y generar token</Button></div></Section>
