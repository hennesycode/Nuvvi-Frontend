              ...
              {logs.map((log) => {
                ...
                return (
                  <tr key={log.id} className="align-top transition-colors hover:bg-[#F8FCFF]/70">
                    ...
                    <td className="px-5 py-4"><p className="font-semibold text-[#102F4B]">{log.actor_name || "Sistema"}</p><p className="text-xs text-[#6C8398]">{log.actor_username ? `@${log.actor_username}` : "Sin usuario"}</p><p className="text-xs text-[#6C8398]">{log.actor_identification || log.actor_email || "-"}</p></td>
